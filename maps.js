const MAP_WIDTH = 700
const MAP_HEIGHT = 610

// Class for the map visualizations
class MapVis {

    constructor(globalApplicationState) {
        // Setup the svg areas
        d3.select("#House-div").append("svg").attr("width", MAP_WIDTH).attr("height", MAP_HEIGHT)
        d3.select("#Senate-div").append("svg").attr("width", MAP_WIDTH).attr("height", MAP_HEIGHT)
        d3.select("#Map-legend-div").append("svg").attr("width", MAP_WIDTH*2).attr("height", 100)

        // Get the application state
        this.globalApplicationState = globalApplicationState;
        const projection = d3.geoAlbersUsa()
            .translate([MAP_WIDTH/2, MAP_HEIGHT/3])
            .scale(900)

        this.path = d3.geoPath()
            .projection(projection)

        this.houseSVG = d3.select("#House-div").select("svg");
        this.senateSVG = d3.select("#Senate-div").select("svg");
        this.legendSVG = d3.select("#Map-legend-div").select("svg");

        this.renderVisualization();
    }

    renderVisualization() {
        // Get the data
        let _this = this;
        let chamber = _this.globalApplicationState.chamber;
        let congress = _this.globalApplicationState.congress;
        let selectedData = this.globalApplicationState.congressData.filter(function (d) {
            if (congress === "all") {
                return d;
            } else {
                return (d.congress == congress)
            }
        })

        let metric = this.globalApplicationState.metric
        let color = d3.scaleSequential(d3.interpolatePurples).domain(d3.extent(selectedData, d => d[metric]));

        let selectedDataSenate = this.globalApplicationState.congressData.filter(function (d) {
            return d.chamber === "Senate"
        })
        let selectedDataHouse = this.globalApplicationState.congressData.filter(function (d) {
            return d.chamber === "House"
        })

        // Rollup data
        let avgDataSenate = d3.rollup(selectedDataSenate, v => d3.mean(v, d => d[metric]), d => d.state_name)
        avgDataSenate = new Map(avgDataSenate);
        let avgDataHouse = d3.rollup(selectedDataHouse, v => d3.mean(v, d => d[metric]), d => d.state_name)
        avgDataHouse = new Map(avgDataHouse);

        this.houseSVG.selectAll("path")
            .data(this.globalApplicationState.mapData.features)
            .join("path")
            .attr("fill", function (d) {
                let value = avgDataHouse.get(d.properties.NAME);
                return color(value);
            })
            .style("stroke", "#000")
            .style("stroke-width", function () {
                let selected = this.__data__.properties;
                if (_this.globalApplicationState.selectedStates.includes(`${selected.NAME}-House`)) {
                    return 5;
                }
                else {
                    return 1;
                }
            })
            .attr("d", _this.path)
            .on("mouseover", function() {
                d3.select(this)
                    .style("stroke", "#fffc02")
                    .style("stroke-width", 5);
                let selected = this.__data__.properties
                _this.showText(selected, avgDataHouse, _this.houseSVG);
            })
            .on("mouseout", function () {
                let selected = this.__data__.properties;
                    if (_this.globalApplicationState.selectedStates.includes(`${selected.NAME}-House`)) {
                        d3.select(this)
                            .style("stroke", "black")
                            .style("stroke-width", 5);
                    }
                    else {
                        d3.select(this)
                            .style("stroke", "black")
                            .style("stroke-width", 1);
                    }
                _this.houseSVG.selectAll('text').remove();
            })
            .on("click", function() {
                let selected = this.__data__.properties;
                if (_this.addState(selected, "House")) {
                    d3.select(this)
                        .style("stroke", "black")
                        .style("stroke-width", 5);
                }
                else {
                    d3.select(this)
                        .style("stroke", "black")
                        .style("stroke-width", 1);
                }
            })

        this.senateSVG.selectAll("path")
            .data(this.globalApplicationState.mapData.features)
            .join("path")
            .attr("fill", function (d) {
                let value = avgDataSenate.get(d.properties.NAME);
                return color(value);
            })
            .style("stroke", "#000")
            .style("stroke-width", function () {
                let selected = this.__data__.properties;
                if (_this.globalApplicationState.selectedStates.includes(`${selected.NAME}-Senate`)) {
                    return 5;
                }
                else {
                    return 1;
                }
                })
            .attr("d", _this.path)
            .on("mouseover", function() {
                d3.select(this)
                    .style("stroke", "#fffc02")
                    .style("stroke-width", 5);
                let selected = this.__data__.properties
                _this.showText(selected, avgDataSenate, _this.senateSVG);
            })
            .on("mouseout", function () {
                let selected = this.__data__.properties;
                if (_this.globalApplicationState.selectedStates.includes(`${selected.NAME}-Senate`)) {
                    d3.select(this)
                        .style("stroke", "black")
                        .style("stroke-width", 5);
                }
                else {
                    d3.select(this)
                        .style("stroke", "black")
                        .style("stroke-width", 1);
                }
                _this.senateSVG.selectAll('text').remove();
            })
            .on("click", function() {
                let selected = this.__data__.properties;
                if (_this.addState(selected, "Senate")) {
                    d3.select(this)
                        .style("stroke", "black")
                        .style("stroke-width", 5);
                }
                else {
                    d3.select(this)
                        .style("stroke", "black")
                        .style("stroke-width", 1);
                }
            })
        this.renderLegend(selectedData,metric)
    }

    addState(selected, chamber) {
        // Add the first selected state to an empty list
        if (this.globalApplicationState.selectedStates.length === 0) {
            d3.select("#congress").property("value", "all");
            d3.select("#chamber").property("value", "both");
            this.globalApplicationState.selectedStates.push(`${selected.NAME}-${chamber}`);
            this.globalApplicationState.mainVis.renderVisualization();
            this.globalApplicationState.colorDict[`${selected.NAME}-${chamber}`] = this.globalApplicationState.count;
            this.globalApplicationState.count = 1;
            return true;
        } else if (this.globalApplicationState.selectedStates.includes(`${selected.NAME}-${chamber}`)) {
            let newArray = this.globalApplicationState.selectedStates.filter((c) => c !== `${selected.NAME}-${chamber}`);
            if (newArray.length === 0) {
                d3.select("#congress").property("value", "all");
                d3.select("#chamber").property("value", "both");
                this.globalApplicationState.selectedStates = [];
                this.globalApplicationState.colorDict = {}
                this.globalApplicationState.mainVis.renderVisualization();
                return false;
            } else {
                this.globalApplicationState.selectedStates = newArray;
                this.globalApplicationState.mainVis.renderVisualization();
                delete this.globalApplicationState.colorDict[`${selected.NAME}-${chamber}`]
                return false;
            }
        } else {
            this.globalApplicationState.selectedStates.push(`${selected.NAME}-${chamber}`);
            this.globalApplicationState.mainVis.renderVisualization();
            this.globalApplicationState.colorDict[`${selected.NAME}-${chamber}`] = this.globalApplicationState.count;
            this.globalApplicationState.count = this.globalApplicationState.count + 1;
            return true;
        }
        }


    showText(selected, avgData, chamberSVG){
        let state = selected.NAME;
        let data = avgData.get(selected.NAME);
        let formatted = d3.format(".2f")(data);
        chamberSVG.selectAll("text").remove()
        chamberSVG.append("text")
            .text(`${state}: ${formatted}`)
            .attr("fill", "black")
            .attr('x', 350)
            .attr('y', 20)
            .attr("font-size", 18)
            .attr("text-anchor", 'middle');
    }

    renderLegend(data, metric) {
        let colorArray = ["#fcfbfd","#fcfbfd","#fbfafc","#fbfafc","#faf9fc","#faf9fc","#faf8fb","#f9f8fb","#f9f7fb","#f8f7fb","#f8f7fa","#f7f6fa","#f7f6fa","#f7f5fa","#f6f5f9","#f6f4f9","#f5f4f9","#f5f3f9","#f4f3f8","#f4f2f8","#f4f2f8","#f3f2f8","#f3f1f7","#f2f1f7","#f2f0f7","#f1f0f7","#f1eff6","#f0eff6","#f0eef6","#efeef5","#efedf5","#eeedf5","#eeecf5","#edecf4","#edebf4","#ecebf4","#ebeaf3","#ebe9f3","#eae9f3","#eae8f3","#e9e8f2","#e8e7f2","#e8e7f2","#e7e6f1","#e7e5f1","#e6e5f1","#e5e4f0","#e5e4f0","#e4e3f0","#e3e2ef","#e3e2ef","#e2e1ef","#e1e1ee","#e1e0ee","#e0dfee","#dfdfed","#dedeed","#dedded","#ddddec","#dcdcec","#dbdbec","#dbdaeb","#dadaeb","#d9d9ea","#d8d8ea","#d7d7ea","#d7d7e9","#d6d6e9","#d5d5e8","#d4d4e8","#d3d3e8","#d2d3e7","#d2d2e7","#d1d1e6","#d0d0e6","#cfcfe5","#cecee5","#cdcee5","#cccde4","#cbcce4","#cbcbe3","#cacae3","#c9c9e2","#c8c8e2","#c7c7e1","#c6c6e1","#c5c5e0","#c4c4e0","#c3c3df","#c2c3df","#c1c2de","#c0c1de","#bfc0dd","#bebfdd","#bebedc","#bdbddc","#bcbcdb","#bbbbda","#babada","#b9b9d9","#b8b8d9","#b7b7d8","#b6b5d8","#b5b4d7","#b4b3d6","#b3b2d6","#b2b1d5","#b1b0d5","#b0afd4","#afaed4","#aeadd3","#aeacd2","#adabd2","#acaad1","#aba9d1","#aaa8d0","#a9a7cf","#a8a6cf","#a7a5ce","#a6a4ce","#a5a3cd","#a4a2cd","#a3a1cc","#a2a0cb","#a19fcb","#a09eca","#9f9dca","#9e9cc9","#9e9ac9","#9d9ac8","#9c99c8","#9b98c7","#9a97c7","#9996c6","#9895c6","#9794c5","#9693c5","#9592c4","#9491c4","#9390c3","#928fc3","#918ec2","#908dc2","#908cc1","#8f8bc1","#8e8ac0","#8d89c0","#8c88bf","#8b87bf","#8a86be","#8985be","#8884bd","#8883bd","#8782bc","#8680bc","#857fbb","#847eba","#837dba","#827cb9","#827bb9","#817ab8","#8079b8","#7f77b7","#7e76b6","#7e75b6","#7d74b5","#7c73b4","#7b71b4","#7b70b3","#7a6fb3","#796eb2","#786cb1","#786bb1","#776ab0","#7668af","#7567af","#7566ae","#7465ad","#7363ad","#7362ac","#7261ab","#715fab","#705eaa","#705ca9","#6f5ba8","#6e5aa8","#6e58a7","#6d57a6","#6c56a6","#6c54a5","#6b53a4","#6a52a4","#6950a3","#694fa2","#684ea2","#674ca1","#674ba0","#664aa0","#65489f","#65479e","#64469e","#63449d","#63439c","#62429c","#61409b","#613f9a","#603e9a","#5f3c99","#5e3b99","#5e3a98","#5d3897","#5c3797","#5c3696","#5b3595","#5a3395","#5a3294","#593194","#582f93","#582e92","#572d92","#562b91","#562a91","#552990","#54288f","#54268f","#53258e","#52248e","#52238d","#51218c","#50208c","#501f8b","#4f1e8b","#4e1c8a","#4e1b8a","#4d1a89","#4c1988","#4c1788","#4b1687","#4a1587","#4a1486","#491286","#481185","#481084","#470f84","#460d83","#460c83","#450b82","#440a82","#440981","#430780","#420680","#42057f","#41047f","#40027e","#40017e","#3f007d"]
        // Set an array to use for data that has index values for the color scale
        let legendData = Array.from(Array(colorArray.length).keys());
        this.legendSVG.selectAll("text").remove();
        // Set the scaling for the bars
        var xScale = d3.scaleLinear()
            .domain([0,colorArray.length-1])
            .range([MAP_WIDTH-75, MAP_WIDTH+75]);

        // Add a rectangle for each color in the color array
        this.legendSVG.selectAll("rect")
            .data(legendData)
            .enter()
            .append("rect")
            .attr("x", (d) => (xScale(d)))
            .attr("y", 20)
            .attr("height", 20)
            .attr("width", 1)
            .attr("fill", (d) => colorArray[d]);

        // Add the labels
        this.legendSVG.append("text")
            .text("0")
            .attr('fill', 'black')
            .attr('x', MAP_WIDTH-75)
            .attr('y', 15)
            .attr('font-size', 18)
            .attr('text-anchor', 'middle');

        // Find the highest value and format it
        let maximumMetric = d3.max(data, d => d[metric]);
        let formatted = d3.format(".2f")(maximumMetric);
        this.legendSVG.append("text")
            .text(formatted)
            .attr('fill', 'black')
            .attr('x', MAP_WIDTH+75)
            .attr('y', 15)
            .attr('font-size', 18)
            .attr('text-anchor', 'middle');
    }
}